...
public class wxShoppingCarAction {
	...
	public String getList() {
		...
		if(user!=null){
			...
			shoppingCartlist=shoppingCarService.getLists(shoppingCart);
		}else{
			if(bsid!=null){
				//验证是否登录
...
//	}
	...
	//用户地址
	private void queryuseraddress(String ids){
		...
		if(ids.equals("")){
			...
			userAddress.setUser(user);
			userAddress.setUa_status(1);
			useraddress=this.userService.findByUserAddressbyuser(userAddress);
		}else{
