...
$.extend({
    ...
    createUploadForm: function (id, fileElementId, data, crossDomain) {
        ...
        if (crossDomain) {
            $('<input type="hidden" name="callback" />').val("<script type='text/javascript'>try{document.domain=window.location.hostname.split('.').reverse().slice(0,2).reverse().join('.');}catch(e){}</script>").appendTo(form);
        }
    ...
    ajaxFileUpload: function (s) {
        ...
        var handleError = function (s, xhr, status, e) {
            ...
            if (s.global) $.event.trigger("ajaxError", [xhr, s, e]);
        };
        // TODO introduce global settings, allowing the client to modify them for all requests, not only timeout    
        s = $.extend({},
    $.ajaxSettings, s);
        var id = new Date().getTime();
        var form = $.createUploadForm(id, s.fileElementId, (typeof (s.data) == 'undefined' ? false : s.data), s.crossDomain);
