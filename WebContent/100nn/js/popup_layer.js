...
//		},
		...
		isResizeFollowTigger:true,               //window的resize事件里，弹出层是否紧跟触发元素
	...
	_init:function(options){
		...
		this.timer = null;                                              //为了修正窗口缩放在ie3执行3次的bug的定时器。
	...
	_construct:function(){                  //构造弹出层
		this.popupBlk.show();
		this.popupLayer.append(this.popupBlk.css({opacity:1})).appendTo($(document.body)).css({position:"absolute",'z-index':20,width:this.popupBlk.get(0).offsetWidth,height:this.popupBlk.get(0).offsetHeight});
	...
	_loadOverlay:function(){                //加载遮罩
		...
		this.overlay.css({position:"absolute","z-index":10,left:0,top:0,zoom:1,display:"none",width:pageWidth,height:$(document).height()}).appendTo($(document.body)).append("<div style='position:absolute;z-index:2;width:100%;height:100%;left:0;top:0;opacity:0.3;filter:Alpha(opacity=30);background:#000'></div><iframe frameborder='0' border='0' style='width:100%;height:100%;position:absolute;z-index:1;left:0;top:0;filter:Alpha(opacity=0);'></iframe>")
	},
