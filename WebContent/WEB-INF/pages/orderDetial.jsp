...
<script type="text/javascript"> 
	$(function(){
		$.ajax({
			   type: "POST",
			   url: "htorderdetail_querycount?order.o_status="+'-1'+"&order.o_sort="+0+"&kddh=3y",
		...
		$.ajax({
			   type: "POST",
			   url: "htorderdetail_querycount?order.o_status="+'8'+"&order.o_sort="+0,
			   success: function(msg){
				    //获取数量
					$("#dfkid span").html(msg);
			   }
		...
		$.ajax({
			   type: "POST",
			   url: "htorderdetail_querycount?order.o_status="+'1'+"&order.o_sort="+0,
			   success: function(msg){
				    //获取数量
					$("#dshid span").html(msg);
			   }
		...
		$.ajax({
			   type: "POST",
			   url: "htorderdetail_querycount?order.o_status="+'0'+"&order.o_sort="+0,
			   success: function(msg){
				    //获取数量
					$("#dfhid span").html(msg);
			   }
		...
		$.ajax({
			   type: "POST",
			   url: "htorderdetail_querycount?order.o_status="+'2'+"&order.o_sort="+0,
			   success: function(msg){
				    //获取数量
					$("#yfhid span").html(msg);
			   }
		...
		$('#grid').datagrid(
				{
					...
					pagination:true,
					pageNumber:1,//初始化页号
					url : "${pageContext.request.contextPath}/htorderdetail_getList?order.o_status="+'-1'+"&order.o_sort="+0,
					idField : 'o_id',
					...
					toolbar : [
								{
									...
									handler:function(){
										...
										if(noids==""){
											...
											$.messager.alert("提示","不能做次操作，选中订单中有未付款的订单","info");
										}
									}
								},{
									id:'plsh',
							...
							formatter:function(value,row,index){
					 		...
					 		formatter:function(value,row,index){
					 				...
					 				if(row.order.o_fkfs==2){
					 					...
					 					str+="   <a href='#'  onclick='zfbfh("+row.order.o_jyh+",\""+a+"\")'>发货<a>";
					 					}
					 				}
	...
	//支付宝发货
	function zfbfh(id,ddbhs){
		
		parent.forward("订单发货","");
		location.href="${pageContext.request.contextPath}/pages_addzfpage?jyh="+id+"&ddbh="+ddbhs;
	}
	//取消订单
	...
	function onplprintcontext(){
		...
		$.ajax({
			   type: "POST",
			   url: "csv_printdata",
			   data: "truename="+truename+"&kddh="+kddh+"&shrinfo="+shrinfo+"&shrdh="+shrdh+"&shdz="+shdz+"&zts="+zts+"&xdstartime="+xdstartime+"&xdendtime="+xdendtime+"&fkstratime="+fkstratime+"&fkendtime="+fkendtime+"&result=0&sortid=0",
			   success: function(msg){
				   $("#contextid").remove();
				   if(msg!=null){
...
<body class="easyui-layout" style="border-top-width:0px;">
	...
	<div region="center" border="false">
		...
		<div class="topfl">
		 <a id="j3y" href="javascript:ontime(3,'j3y');">近三个月（<span>0</span>）</a> | <a id="dfkid" href="javascript:onstaic(8,'dfkid');">待付款（<span>0</span>）</a> | <a id="dshid" href="javascript:onstaic(1,'dshid');">待收货（<span>0</span>）</a> | <a id="dfhid" href="javascript:onstaic(0,'dfhid');">待发货（<span>0</span>）</a> | <a id="yfhid" href="javascript:onstaic(2,'yfhid');">已收货（<span>0</span>）</a> | <a id="3yold" href="javascript:ontime(-3,'3yold');">三个月前（<span>0</span>）</a> 
		</div>
			<div class="serachdiv" >
		<!-- 	订单编号:<input type="text" id="truename" name="" value="">
                ...
                订单编号：<input name="txtOrderNo" type="text" maxlength="20" id="truename" class="txt200">&nbsp; 快递单号：<input name="txtTrackNo" type="text" maxlength="50" id="txtTrackNo" class="txt200" >&nbsp;
                <br>
                <br>
