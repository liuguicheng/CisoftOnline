	...
	$(function() {
		...
		$("#savecgid").click(function(){
			...
			$("#storeroom").change(function(){
			   var stid=$("#storeroom").val();
			   $.ajax({
		            cache: true,
		            type: "POST",
		            data:"spbh="+stid,
		            url:"purchase_quyerstoreroom",
		            success: function(data) {
		            	$("#shdz").val(data);
		            }
		        });
...
<body class="easyui-layout" style="border-top-width:0px;">
 ...
 <tr> 
   <td>收货地址</td>
   <td colspan="5"><input type="text" name="p_jhAddres" id="shdz" value="" style="width: 668px;"><span class="span_1">＊请输入收货地址</span></td>
 </tr>
 <tr> 
   <td>备注</td>
   <td colspan="5"><input type="text" name="p_ms" style="width: 600px;">
   <input  type="button" value="保   存" class="input_buttion" id="savecgid" style="margin-left: 112px;"/>
	...
	<input type="hidden" id="pi_no" id="pi_no" name="pi_no" />
	<input type="hidden" id="pi_scbs" name="pi_scbs" value="${pi_scbs }"/>
		<input type="hidden" name="commodity.c_id" id="comid" value=""/>
	<table  cellpadding="10" cellspacing="1" class="table1">
 <tr> 
 <td>商品条形码</td>
   ...
   <td>进货价</td>
   <td><input type="text" name="pi_cgprice" value=""></td>
 </tr>
 <tr> 
   <td>数量</td>
   <td><input type="text" name="pi_num" id="numid"><span class="span_1">＊请输入采购数量</span></td>
    <td>供应商</td>
   <td >
