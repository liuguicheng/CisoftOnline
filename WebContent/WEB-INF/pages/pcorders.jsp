...
<script type="text/javascript"> 
	$(function(){
		$.ajax({
			   type: "POST",
			   url: "htorderdetail_querycount?order.o_status="+'-1'+"&order.o_sort="+1+"&kddh=3y",
		...
		$.ajax({
			   type: "POST",
			   url: "htorderdetail_querycount?order.o_status="+'2'+"&order.o_sort="+1,
			   success: function(msg){
				    //获取数量
					$("#yfhid span").html(msg);
			   }
		...
		var urlstr="${pageContext.request.contextPath}/htorderdetail_getList?order.o_status="+$("#o_status").val()+"&order.o_sort="+1;
		var s=$("#o_status").val();
		if(s==-1){
			urlstr=urlstr;
		}else{
			urlstr="${pageContext.request.contextPath}/htorderdetail_getList?order.o_status="+$("#o_status").val()+"&order.o_sort="+1;
		}
		$('#grid').datagrid(
				{
	...
	function zfbfh(id,ddbhs){
		...
		var widoid=",'new"+id+"'";
		  var iWidth="824"; //弹出窗口的宽度;
			 ...
			 var iTop = (window.screen.availHeight-30-iHeight)/2; //获得窗口的垂直位置;
			 var iLeft = (window.screen.availWidth-10-iWidth)/2; //获得窗口的水平位置;
	         window.open('${pageContext.request.contextPath}/pages_addzfpage?jyh='+id+'&ddbh='+ddbhs,widoid,'height='+iHeight+',width='+iWidth+',top='+iTop+',left='+iLeft+',toolbar=no,menubar=no,scrollbars=no,resizable=no,location=no,status=no');
	...
	function onplprintcontext(){
		...
		$.ajax({
			   type: "POST",
			   url: "csv_printdata",
			   data: "truename="+truename+"&kddh="+kddh+"&shrinfo="+shrinfo+"&shrdh="+shrdh+"&shdz="+shdz+"&zts="+zts+"&xdstartime="+xdstartime+"&xdendtime="+xdendtime+"&fkstratime="+fkstratime+"&fkendtime="+fkendtime+"&result=0&sortid=1",
			   success: function(msg){
				   $("#contextid").remove();
				   if(msg!=null){
