...
function getData(){
    ...
    $.ajax({
        type:"post",//设置get请求方式
        url:"wxcommodity_querycommdityListajax?commodity.rows=4&commodity.page=0",//设置请求的脚本地址
        data:"",//设置请求的数据
        dataType:"json",//设置请求返回的数据格式
        success:function(data){
            var pubu = $("#pubu");
            $.each(data.rows, function (i, item) {
                var box = $("<div class='box'><div class='pic'><img src=../imgss.jsp?file="+item.c_sltimg+"></div></div>");
                pubu.append(box);
